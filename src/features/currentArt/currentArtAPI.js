...
export const artApi = createApi({
  reducerPath: 'artApi',
  baseQuery: fetchBaseQuery({ baseUrl: `https://api.harvardartmuseums.org/object/`}),
  endpoints: (builder) => ({
    getArtById: builder.query({
      query: (id) => `${id}/?apikey=b5915d6a-dcba-45df-a4a6-9ff3a72dfbeb`,
    }),
  }),
})

export const { useGetArtByIdQuery } = artApi
